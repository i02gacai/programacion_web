...
<title> ${sessionScope.nombre} - Modifica la pista</title>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/habilitarCampo.js"></script>
</head>
<body>
...
<%@include file="../../../includes/header_admin.jsp" %>
	<div class="login">
		...
		<form onsubmit="return validarFormatoPassword()" id="myForm" action="/practica3/modificarPistaControllerServlet" method="post">
			...
			<% Object kartsObj = pageContext.findAttribute("lista_pistas");
		   
		   // Verificamos si el objeto es de tipo ArrayList<Integer> y lo asignamos a la variable 'lista'
		   pistaBean lista = new pistaBean();
		   if (kartsObj instanceof pistaBean) {
		       lista = (pistaBean) kartsObj;
		   }
%>

			<label class="container">ID: <select name="id_pista"> 
			
			<%for(int i=0;i<lista.getPistas().size();i++){ %>
				<option value="<%= lista.getPistas().get(i).getId_pista() %>_<%= lista.getPistas().get(i).getNomPista() %>" > <%= lista.getPistas().get(i).getNomPista() %></option> <% } %>
			</select>
			</label>
