...
<% Object pistaObj = pageContext.findAttribute("lista_pistas");
	...
	%>
		...
		<form onsubmit="return validarFormatoPassword()" id="myForm" action="/practica3/realizarReservaBonoControllerServlet" method="post">
			        ...
			        <select id="seleccionFormulario" name="seleccion" required="required" disabled onchange="mostrarCampos()">
			            <option value="<%= request.getParameter("seleccion") %>"><%= request.getParameter("seleccion") %></option>
			        </select>
			        ...
			        <input type="hidden" name="seleccion" value="<%= request.getAttribute("seleccion") %>"/>				 
